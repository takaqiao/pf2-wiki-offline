...
- [x] **P6 发版**: **DONE(2026-05-21)**。用户授权 → `release.ps1 -PrevVer v0.3.23 -NewVer v0.3.24`(复用 v0.3.23 exe,纯内容)。补丁 +57 新/~420 改/-0,**21.91 MB(98.2% 省)**。tag v0.3.24 + GitHub release(portable 1218MB + patch 21.9MB + patches.json `latest=v0.3.24`、chain v0.3.23→v0.3.24)+ repo commit `c0b68f3` 已推送。**🎉 全任务 P0–P6 DONE。**
...
- 2026-05-21(iter8) | **P6 DONE — v0.3.24 已发布**。用户「同意」授权后跑 release.ps1:补丁 21.91MB(+57/~420 改,98.2% 省),tag+release+patches.json(latest=v0.3.24)+ repo commit c0b68f3 全推送,3 资产齐(portable 1218MB/patch 21.9MB/patches.json)。**🎉 分类/子区修复全任务结束:A 分类驱动、B 已正确、C 数据驱动子区,staleness 经 revision-refresh 消除(残留 ~0.2% 不可约)。** 遗留(范围外/低优):letter 桶(browse-CJK/C)88 既存死链(build_browse_letters_v2,非分类/子区)。
- **[已到发布前]** 全量重建完成(refreshed corpus);终验:A verify 全 staleness 级(0 污染)、C 全 ⊆ 父桶、我改页 0 死链(letter 桶 88 既存死链范围外)、staleness 已 refresh(残留 ~0.2%=live 持续编辑+模板驱动分类变更不可约);4 版本文件已 bump **v0.3.24**(未提交,release.ps1 会提交)。`release.ps1` 被 auto 模式拦截(公开发布需用户显式授权)。**待用户**:批准跑 `release.ps1 -PrevVer v0.3.23 -NewVer v0.3.24`(复用 v0.3.23 exe)或自行运行。
