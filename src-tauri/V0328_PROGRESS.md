...
## ② 延后打磨(P3 小批)
- [ ] 搜索结果键盘导航(↑↓/Enter/Esc + roving active row)search.js。
- [ ] lightbox:图注 caption + 点图不关闭(只背景/按钮关)image_lightbox.js。
- [ ] 移动抽屉 a11y:Esc 关 + aria-expanded + resize 反应 mw_collapsible.js。
- [ ] 快捷键帮助浮层(? 触发,列 Ctrl+K/T/B 等)keybindings.js。
...
- [ ] scraper 退避重试(dump_parsed 429/5xx backoff)— 廉价robust。
